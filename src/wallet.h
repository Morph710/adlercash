...
enum AvailableCoinsType {
    ...
    ADLY_DENOMINATED = 2,
    ADLY_NOT10000IFMN = 3,
    ADLY_NONDENOMINATED_NOT10000IFMN = 4, // ADLY_NONDENOMINATED and not 10000 ADL at the same time
    ADLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zADL send
enum ZerocoinSpendStatus {
    ZADL_SPEND_OKAY = 0,                            // No error
    ZADL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZADL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZADL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZADL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZADL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZADL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZADL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZADL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZADL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZADL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZADL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZADL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZADL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZADL_SPENT_USED_ZADL = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZADLAutoBackups(bool fEnabled)
    {
...
public:
    ...
    CAmount GetDebit(const isminefilter& filter) const
    {
        ...
        if (filter & ISMINE_WATCH_ADLY) {
            if (fWatchDebitCached)
                debit += nWatchDebitCached;
            else {
                nWatchDebitCached = pwallet->GetDebit(*this, ISMINE_WATCH_ADLY);
    ...
    CAmount GetCredit(const isminefilter& filter) const
    {
        ...
        if (filter & ISMINE_WATCH_ADLY) {
            if (fWatchCreditCached)
                ...
                nWatchCreditCached = pwallet->GetCredit(*this, ISMINE_WATCH_ADLY);
    ...
    CAmount GetImmatureWatchOnlyCredit(const bool& fUseCache = true) const
    {
        if (IsCoinBase() && GetBlocksToMaturity() > 0 && IsInMainChain()) {
            ...
            nImmatureWatchCreditCached = pwallet->GetCredit(*this, ISMINE_WATCH_ADLY);
    ...
    CAmount GetAvailableWatchOnlyCredit(const bool& fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            if (!pwallet->IsSpent(GetHash(), i)) {
                const CTxOut& txout = vout[i];
                nCredit += pwallet->GetCredit(txout, ISMINE_WATCH_ADLY);
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableCredit() : value out of range");
            }
        }
