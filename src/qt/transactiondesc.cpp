...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
    ...
    if (wtx.IsCoinBase() && nCredit == 0) {
        ...
        if (fAllFromMe) {
            if (fAllFromMe == ISMINE_WATCH_ADLY)
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty()) {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address)) {
                        ...
                        if (toSelf == ISMINE_SPENDABLE)
                            strHTML += " (own address)";
                        else if (toSelf == ISMINE_WATCH_ADLY)
                            strHTML += " (watch-only)";
                        strHTML += "<br>";
                    }
                }
    ...
    // Message from normal AdlerCash:URI (AdlerCash:XyZ...?message=example)
        ...
        BOOST_FOREACH (const CTxIn& txin, wtx.vin) {
            ...
            if (pcoinsTip->GetCoins(prevout.hash, prev)) {
                if (prevout.n < prev.vout.size()) {
                    ...
                    strHTML = strHTML + " IsWatchOnly=" + (wallet->IsMine(vout) & ISMINE_WATCH_ADLY ? tr("true") : tr("false")) + "</li>";
                }
            }
        }
