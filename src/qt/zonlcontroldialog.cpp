...
#include "zADLcontroldialog.h"
#include "ui_zADLcontroldialog.h"
...
std::list<std::string> ZADLControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZADLControlDialog::listMints;

ZADLControlDialog::ZADLControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZADLControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZADLControlDialog::~ZADLControlDialog()
{
    delete ui;
}

void ZADLControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZADLControlDialog::updateList()
{
...
void ZADLControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZADLControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZADL_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZADLControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZADLControlDialog::GetSelectedMints()
{
...
void ZADLControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
