                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zADL ought to be enough for anybody." - Bill Gates, 2017
    ui->zADLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzADLSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzADL->setEnabled(false);
        ui->pushButtonMintzADL->setToolTip(tr("zADL is currently disabled due to maintenance."));

        ui->pushButtonSpendzADL->setEnabled(false);
        ui->pushButtonSpendzADL->setToolTip(tr("zADL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zADLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzADL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zADL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zADL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zADL in ") + 
...
void PrivacyDialog::on_pushButtonSpendzADL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zADL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zADL
        sendzADL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zADL
    sendzADL();
}

void PrivacyDialog::on_pushButtonZADLControl_clicked()
{
    ZADLControlDialog* zADLControl = new ZADLControlDialog(this);
    zADLControl->setModel(walletModel);
    zADLControl->exec();
}

void PrivacyDialog::setZADLControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzADLSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzADL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid AdlerCash Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zADLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zADL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ADL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zADL</b>";
    ...
    // use mints from zADL selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZADLControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZADLControlDialog::GetSelectedMints();
    }
    ...
    ZADLControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zADL Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ADL, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zADL Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zADL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zADL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zADL "));
    ui->labelzADLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzADLSyncStatus->setVisible(fShow);
}
